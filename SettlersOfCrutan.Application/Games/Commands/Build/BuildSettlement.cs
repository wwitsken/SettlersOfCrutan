...
public record BuildSettlementCommand(GameId GameId, PlayerId PlayerId, Vertex Vertex) : ICommand<PopulationCenter>;

public sealed class BuildSettlementCommandHandler(IGameRepository gameRepository, StandardPriceCalculator priceCalculator) : ICommandHandler<BuildSettlementCommand, PopulationCenter>
{
    private readonly IGameRepository _gameRepository = gameRepository;
    private readonly StandardPriceCalculator _priceCalculator = priceCalculator;

    public async Task<Result<PopulationCenter>> Handle(BuildSettlementCommand command, CancellationToken ct = default)
    {
