...
using SettlersOfCrutan.Domain.Specifications.Maritime2to1Trade;

namespace SettlersOfCrutan.Domain.UnitTests.Specifications;

public class Maritime2to1TradeSpecTests
{
    private static Maritime2to1TradeContext MakeContext(
        ...
        Board? board = null)
    {
        ...
        return new Maritime2to1TradeContext(
            phase,
            currentPlayerId ?? pid,
            pid,
    ...
    [Fact]
    public void PlayerMustHaveMatching2to1Port_NoSettlementOnPort_Fails()
    {
        var pid = new PlayerId { Value = "p1" };
        var result = new PlayerMustHaveMatching2to1Port().IsSatisfiedBy(
            MakeContext(board: new Board(), actingPlayerId: pid, currentPlayerId: pid, discard: ResourceCardType.Brick));
        Assert.True(result.IsFailure);
