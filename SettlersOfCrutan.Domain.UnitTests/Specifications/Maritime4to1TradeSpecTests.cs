...
using SettlersOfCrutan.Domain.Specifications.Maritime4to1Trade;

namespace SettlersOfCrutan.Domain.UnitTests.Specifications;

public class Maritime4to1TradeSpecTests
{
    private static Maritime4to1TradeContext MakeContext(
        ...
        ResourceHand? bank = null)
    {
        ...
        return new Maritime4to1TradeContext(
            phase,
            currentPlayerId ?? pid,
            pid,
    ...
    [Fact]
    public void PlayerMustHaveDiscardResources_HasFourOrMore_Succeeds()
    {
        var player = Player.Create(TestIds.User(1));
        player.AddResource(ResourceCardType.Brick, 4);
